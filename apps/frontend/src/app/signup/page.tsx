    ...
    <div className="flex min-h-[calc(100vh-5rem)] items-center justify-center bg-[#FAFAFA] relative overflow-hidden">
      ...
      <div className="w-full max-w-md p-10 space-y-8 bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] border border-white/50 z-10 relative">
        <div className="text-center">
          <div className="w-12 h-12 mx-auto rounded-full bg-black flex items-center justify-center text-white font-bold mb-6">
            <div className="w-4 h-4 rounded-full bg-white/20 ml-1" />
          </div>
          <h2 className="text-3xl font-black text-black tracking-tight">Join Us.</h2>
          <p className="mt-3 text-sm font-medium text-zinc-500">Create an account instantly.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-10 space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full flex items-center justify-center gap-2 py-3.5 px-4 border border-transparent rounded-xl shadow-md bg-[#0D1F43] hover:bg-[#0A1835] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#0D1F43] transition-all duration-300 active:scale-[0.98] disabled:opacity-50"
          >
            <span className="text-sm font-bold text-white">{loading ? "Creating account..." : "Sign up"}</span>
          </button>
        </form>
        ...
        <div>
          ...
          >
            ...
            <span className="text-sm font-bold text-black">Sign up with Google</span>
          </button>
        </div>

        <div className="mt-8 text-center text-sm font-medium text-zinc-500">
          Already have an account?{" "}
          <Link href="/login" className="font-bold text-black hover:text-zinc-700 transition-colors border-b border-black pb-0.5">
            Log in
          </Link>
        </div>
      </div>
